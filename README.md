...
- Column sweeper clears its column; colour sweep clears its own colour; number sweep clears its own face value; bomb clears a clipped 3×3; coin awards one coin.
- Blast-only dice score pips × (1 + cascade depth), with no size or low-number bonus.
- Each die has a 2% chance of each of the five specials, mutually exclusive: 10% combined. Colours and faces are uniform independently. Initial boards are conditioned on having no pre-existing matches.
...
Eighteen automated tests pass, including 250 seeded board sequences. Whole-game modelling covers 1,000 baseline runs (250 per policy) and 3,200 sensitivity runs (16 configurations × two policies × 100 seeds). Every example trace is replayed, and every run checks score/coin accounting. No action or cascade ceilings were reached.

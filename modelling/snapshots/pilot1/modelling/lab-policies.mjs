...
export const LAB_POLICIES=['lab-flex','lab-cascade','lab-four','lab-five','lab-six','lab-special','lab-wild','lab-spectrum'];
export function labChoose(name,v) {
 ...
 const ranks=t=>{
  ...
  if(large)return {quad:14,convert:30,ones:3,cascade:3,rainbow:has('convert')?0:4,bigbomb:0,widecolumn:0,widerow:0,sculptor:29,spare:17,resonator:rate('wild')?8:0,conductor:0}[t.id]??0;
 ...
 const swaps=legalActions(v.board).map(action=>({action,f:previewAction(v.board,action,v.config)})).sort((a,b)=>b.f.score-a.f.score);
 const best=swaps[0];
 if((v.singleRerollsUsed??0)<rerollLimit(v.config)){
  if(r.lab&&has('sculptor')&&!v.sculptUsed){
   ...
   for(let index=0;index<36;index++)if(!v.board[index].special)for(let from=0;from<36;from++){
    ...
    const f=previewBoard(b,v.config),big=f.groups.some(g=>g.length>=(build==='six'?6:5));
  ...
  for(let index=0;index<36;index++){
   ...
   for(let face=1;face<=6;face++){b[index].n=face===6&&has('convert')?1:face;b[index].color=b[index].n-1;const f=previewBoard(b,v.config);expected+=f.score/6;big+=Number(f.groups.some(g=>g.length>=(build==='six'?6:5)))/6;}

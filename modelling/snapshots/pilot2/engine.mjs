...
const TRINKETS = [...MATCH_TIERS.flatMap(tier => ["pips", "mult"].map(stat => ({
  ...
  {id:'sculptor',name:'Mimic Ring',research:true,description:'Once per encounter, spend a reroll to copy an adjacent numbered face'},
...
const trinketCost=(item,config)=>config.rules?.lab?.prices?.[item.id]??(item.tier?rulesFor(config).trinketCost:rulesFor(config).charmPrices[item.id]??12);
const rerollLimit=config=>rulesFor(config).singleRerollsPerRound+(config.rules?.lab&&has(config,'spare')?(config.rules.lab.extraRerolls??1):0);
...
function wave(b, groups, depth, config, roots = []) {
  ...
  if(rules.lab&&has(config,'cluster')) {
    ...
    for(const group of groups){
      const n=group.map(i=>b[i]).find(numbered)?.n??6,set=new Set(group),queue=[...group];
      for(let at=0;at<queue.length;at++)for(const i of [queue[at]-6,queue[at]+6,queue[at]-1,queue[at]+1])
        if(adjacent(queue[at],i)&&!set.has(i)&&numbered(b[i])&&b[i].n===n){set.add(i);queue.push(i);}
  ...
  for (const g of groups) {
    ...
    const trinkets=TRINKETS.filter(t=>t.tier===tier && config.trinkets?.includes(t.id)&&!(t.stat==='mult'&&config.rules?.lab?.tierX?.[tier])).map(t=>({...t,value:trinketValue(t,config)}));
...
function act(original, action) {
  ...
  if (action.type === "swap") {
    ...
    if (!Number.isInteger(action.index) || action.index < 0 || action.index > 35 ||
        s.board[action.index].special || (s.singleRerollsUsed ?? 0) >= rerollLimit(s.config)) return null;
    ...
    if(!rules.lab||!has(s.config,'sculptor')||s.sculptUsed||!Number.isInteger(action.index)||!Number.isInteger(action.from)||
      action.index<0||action.index>35||action.from<0||action.from>35||!adjacent(action.index,action.from)||
  ...
  const batteryRefund=rules.lab?.battery&&frames.some(f=>f.activations.some(a=>a.type==='twenty'))?Math.min(1,s.singleRerollsUsed??0):0;
  if(batteryRefund)s.singleRerollsUsed-=batteryRefund;

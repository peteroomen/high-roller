...
import {newGame,act,restoreGame,DEFAULTS,TYPES,wave,legalActions,canTakeToken} from '../engine.mjs';
...
test('zero base rates; deterministic free choice of one from three starter tokens',()=>{
 ...
 assert.equal(out.state.config.rates[TYPES.indexOf(chosen)],5);assert.equal(out.state.config.rates.reduce((a,b)=>a+b,0),5);
...
test('shop pays once, charges once, keeps exactly three of five and resumes next round',()=>{
 ...
 assert.equal(s.status,'shop');assert.equal(s.config.rates.reduce((a,b)=>a+b,0),20);
...
test('two three-die groups add their multipliers over all pips',()=>{
 ...
 assert.equal(f.pips,27);assert.equal(f.mult,2);assert.equal(f.score,54);
});
...
test('complete campaigns replay every draft, purchase and round transition',()=>{
 const config={...DEFAULTS,targets:[1,1,1,1,1,1]};
 const r=runOne({seed:24,policy:'greedy',config});
 assert.equal(r.status,'won');assert.equal(r.packPurchases,5);assert.equal(r.shopVisits,5);
 assert.equal(Object.values(r.tokenPicks).reduce((a,b)=>a+b,0),16);
